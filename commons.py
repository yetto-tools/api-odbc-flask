...
def send_to_xlsx_fix(data, file_name="Reporte Especiales2 Contado"+ str(datetime.today()).replace(":","")+".xlsx", sheet_name="Especiales_2", headers=None):
    ...
    for date,day_of_week,customer,name,amount,payment_method in data:
        ...
        if [name,date] not in temp:
            ...
            #duplicate_name.append([subindex,date,day_of_week,customer,name,str(amount),payment_method])
...
def send_to_simple_xlsx_fix(data, file_name="Reporte Especiales2 Contado"+ str(datetime.today()).replace(":","")+".xlsx", sheet_name="Especiales_2", headers=None):
    ...
    for DIA in COLUMNAS:
        ...
        worksheet.write_formula(COLUMNAS[DIA]+str(index+2), '=SUM('+COLUMNAS[DIA]+str(1)+':'+COLUMNAS[DIA]+str(index)+")",bold )
...
# cursor = conn.cursor()
# result = cursor.execute(ESPECIALES_2_TO_EXCEL, '2022-06-13', '2022-06-18',0)
# data = result.fetchall()
# import pprint 
# pprint.pprint(data)


# send_to_xlsx_fix(data, headers=['fecha', 'COLUMNAS[DIA]', 'cliente', 'nombre', 'monto', 'forma'])
